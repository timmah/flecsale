...
template <std::size_t N> using mesh_t = typename flecsale::mesh::burton::burton_mesh_t<N>;
...
template <std::size_t N> using matrix_t = flecsale::math::matrix<real_t, N, N>;

using eos_t = flecsale::eos::eos_base_t<real_t>;

template <std::size_t N> using eqns_t = flecsale::eqns::lagrange_eqns_t<real_t, N>;

template <std::size_t N> using flux_data_t = typename eqns_t<N>::flux_data_t;
...
//!   update errors.
enum class solution_error_t { unphysical, variance, ok };

//! \brief a class to distinguish between different types of
//!   restarting modes.
enum class mode_t { normal, retry, restart, quit };

////////////////////////////////////////////////////////////////////////////////
//! \brief A general boundary condition type.
//! \tparam N  The number of dimensions.
////////////////////////////////////////////////////////////////////////////////
template <std::size_t N> class boundary_condition_t {
public:
  using real_type = real_t;
  using vector_type = flecsale::math::vector<real_type, N>;

  virtual bool has_prescribed_velocity() const { return false; };

  virtual bool has_prescribed_pressure() const { return false; };

  virtual bool has_symmetry() const { return false; };
...
public:
  virtual bool has_symmetry() const override { return true; }
...
////////////////////////////////////////////////////////////////////////////////
template <typename M> class cell_state_accessor {
public:
  //! some type aliases
  ...
  template <typename T>
  using accessor_t = decltype(
      flecsi_get_accessor(std::declval<M>(), hydro, var_name, T, dense, 0));
  ...
  constexpr cell_state_accessor(M &mesh)
      : m(flecsi_get_accessor(mesh, hydro, cell_mass, real_t, dense, 0)),
        ...
        e(flecsi_get_accessor(mesh, hydro, cell_internal_energy, real_t, dense,
                              0)),
        t(flecsi_get_accessor(mesh, hydro, cell_temperature, real_t, dense, 0)),
        a(flecsi_get_accessor(mesh, hydro, cell_sound_speed, real_t, dense,
                              0)) {}
  ...
  template <typename T> constexpr auto operator()(T &&i) const {
    ...
    return std::forward_as_tuple(
        V[forward<T>(i)], m[forward<T>(i)], v[forward<T>(i)], p[forward<T>(i)],
        d[forward<T>(i)], e[forward<T>(i)], t[forward<T>(i)], a[forward<T>(i)]);
  }
  ...
  //! \param [in] i  the element we are accessing
  //! \return a tuple of references to all the state data
  template <typename T> auto operator()(T &&i) {
    using std::forward;
    return std::forward_as_tuple(
        V[forward<T>(i)], m[forward<T>(i)], v[forward<T>(i)], p[forward<T>(i)],
        d[forward<T>(i)], e[forward<T>(i)], t[forward<T>(i)], a[forward<T>(i)]);
